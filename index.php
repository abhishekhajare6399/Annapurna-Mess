...
<div class="content">
<h3><?php echo $mess?></h3>
  <p>Login Your self With <?php echo $mess?> System.</p>
...
if(isset($_POST['submit']))
{
    ...
    $time = $email_pass['time'];
    $point = $email_pass['point'];
    $messtype = $email_pass['messtype'];
    if($messtype=="Non-Regular"){
      $date = date("Y-m-d");
      if($time=="Both"){
      if($point%2!=0) $point = ($point+1)/2;
      else $point = $point/2;
      }else{
        if($point%2!=0) $point = ($point+1);
      else $point = $point;
      }
      $edate = date("Y-m-d" ,strtotime($date . " +" .$point." days"));
      $updatequery = "update user  set end_date='$edate' where userid='$userid' and messid='$messid' ";
